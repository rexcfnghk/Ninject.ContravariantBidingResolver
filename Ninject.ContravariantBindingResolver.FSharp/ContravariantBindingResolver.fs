    ...
    interface IBindingResolver with
        member __.Resolve(bindings, service) =
            ...
            then Seq.empty
                ...
                if genericArguments.Length = 1 || not <| genericArguments.[0].GenericParameterAttributes.HasFlag GenericParameterAttributes.Contravariant
                then Seq.empty
                else
                    let argument = service.GetGenericArguments () |> Array.exactlyOne
                    bindings
                    |> Seq.map (fun (KeyValue (k, v)) -> k, k.GetGenericArguments () |> Array.exactlyOne, v)
                    |> Seq.filter (fun (key, genericArgument, _) -> key.IsGenericType && key.GetGenericTypeDefinition() = genericType && genericArgument <> argument && genericArgument.IsAssignableFrom argument)
